 ...
 */
    ...
    _canHandleAIReply(frame, nodeAddr) {
        ...
        if (cmd === 0x09) {
            ...
            const urv = (((frame.data[2] << 8) | frame.data[3])) / 100;
            const lrv = (((frame.data[4] << 8) | frame.data[5])) / 100;
        ...
        if (cmd === 0x07) {
            const chIdx = frame.data[1] & 0x03;
            const chId = chKeys[chIdx];
            const hh = (((frame.data[2] << 8) | frame.data[3])) / 10;
            const ll = (((frame.data[4] << 8) | frame.data[5])) / 10;
            if (!this.data.ai[chId]) this.data.ai[chId] = {};
        ...
        if (cmd === 0x08) {
            const chIdx = frame.data[1] & 0x03;
            const chId = chKeys[chIdx];
            const h = (((frame.data[2] << 8) | frame.data[3])) / 10;
            const l = (((frame.data[4] << 8) | frame.data[5])) / 10;
            if (!this.data.ai[chId]) this.data.ai[chId] = {};
            if (!this.data.ai[chId].alarms) this.data.ai[chId].alarms = {};
        ...
        if (cmd === 0x0A) {
            ...
            const alarmText = { LL: '低低限', L: '低限', H: '高限', HH: '高高限' };
    ...
    _renderNetworkPage() {
        ...
        [1, 2, 3, 4].forEach(addr => {
            ...
            // 获取该节点的 NMT 状态（从 this.nmtNodeStates）
            const nodeTypeMap = { 1: 'ai', 2: 'ao', 3: 'di', 4: 'do' };
            const nodeType = nodeTypeMap[addr];
            const nmtState = this.nmtNodeStates[nodeType] || 'init';
            ...
            // 改进的状态判断：
            // 1. 如果 NMT 状态是 RUN 但从未收到帧 → "RUN (no data)"
            // 2. 如果 NMT 状态是 RUN 但超时 → "TIMEOUT"
            // 3. 如果总线在线且有帧 → "ONLINE"
            // 4. 否则按原逻辑处理
            let statusStr;
            let statusColor;
            if (nmtState === NMT_STATE.RUN) {
                if (neverSeen) {
                    statusStr = 'RUN (no data)';
                    statusColor = C.yellow;  // 未收到数据时用黄色提示
                } else if (timeout) {
                    statusStr = 'TIMEOUT';
                    statusColor = C.red;
                } else {
